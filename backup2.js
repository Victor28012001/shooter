...
function buildLevel(level) {
  ...
  for (const box of level.objects) {
    ...
    const boxShape = new CANNON.Box(
      new CANNON.Vec3(roomWidth / 2, wallHeight, roomDepth / 2)
    );
...
loader.load("./models/low_poly_abandoned_brick_room-opt.glb", function (gltf) {
  ...
  // Create the static room body
  roomBody = new CANNON.Body({
    mass: 0, // Static body
    position: new CANNON.Vec3(0, 0, 0),
  });
  ...
  roomBody.addShape(boxShape);

  roomBody.collisionResponse = true;
  roomBody.name = "Room";

  roomBody.updateBoundingRadius();
  roomBody.updateMassProperties(); 
  roomBody.updateInertiaWorld();
  roomBody.sleepSpeedLimit = 0;
  roomBody.sleepTimeLimit = 9999;
  roomBody.aabbNeedsUpdate = true;
...
function animate() {
  ...
  roomBody.position.copy(abandonedBuilding.position);
  // roomBody.quaternion.copy(abandonedBuilding.position);
...
function spawnAnimals() {
  ...
  totalSpiders = 2; // Limit to 12 spiders
  spawnedSpiders = 0;
